...
public:
   ...
   */
   ...
   /**
      Sets the size of this clock.
      @param width the width of the enclosing window
      @param height the width of the enclosing window
   */              
...
void Clock::set_size(int width, int height)
{
   centerx = width / 2;
   centery = height / 2;
   if (width < height)
...
void ClockWindow::OnPaint(wxPaintEvent& event)
{
   ...
   wxSize size = GetSize();
   clock.set_size(size.GetWidth(), size.GetHeight());
